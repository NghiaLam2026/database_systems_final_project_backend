...
- **Frontend**: React.js single-page application (lives in a separate repository).
- **Backend** (this repo): FastAPI service exposing REST APIs.
...
The FastAPI backend is organized into modules:
...
## 6. Frontend (Separate Repository)

The React.js frontend lives in its own repository and communicates with this backend exclusively over REST APIs. See the frontend repo for UI implementation details.

**Key integration points:**

- `GET /catalog/...` for hardware library browsing.
- `GET/POST/PATCH/DELETE /builds` and `/builds/{id}/parts` for build workspace.
- `GET/POST /threads` and `/threads/{id}/messages` for AI chat.
- The backend hides the multi-agent mechanics; the frontend sees a single endpoint for sending a chat message and receiving a reply.
...
- The FastAPI backend and a separate React frontend coordinate to provide authenticated user and admin experiences.
